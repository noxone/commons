...
public abstract class FXMLWindowLoader {
	/**
	 * Expected extension of FXML files. If the filename of an XML file is
	 * anticipated by a controller class name, the file is expected to have this
	 * extensions.
	 */
	 ...
	 * <li>If the controller extends {@link AbstractWindowController} the
	 * controllers extra functionality will be initialized</li>
	 * <li>add icons to the stage (based on the given string format)</li>
	 * </ul>
	 *
	 * @param <T>             the type of the controller class
	 * @param controllerClass the controller class to control the window
	 * @param stage           the stage to use for the scene
	 * @param fxmlFileName    the filename of the FXML file to load. This file name
	 *                        must be relative to the <code>controllerClass</code>
	 *                        as {@link Class#getResourceAsStream(String)} will be
	 *                        called on that object.
	 * @param iconFileFormat  a format to generate filenames depending on a icon
	 *                        size. Use a format like <code>icon_%s.png</code>. This
	 *                        format will be used with several integer values in
	 *                        order to generate filename. With this filename
	 *                        {@link Class#getResourceAsStream(String)} will be
	 *                        called on <code>controllerClass</code> in order to
	 *                        load an icon. If the an icon cannot be loaded it will
	 *                        be discarded.
	 * @return
	 */
	public static <T> T load(final Class<T> controllerClass,
			final Stage stage,
			final String fxmlFileName,
			final String iconFileFormat) {
		return load(stage, () -> controllerClass.getResourceAsStream(fxmlFileName),
				size -> getResourceAsStream(controllerClass, String.format(iconFileFormat, size)));
	}

	/**
	 * Load a window with default settings. This method does a lot of magic:
	 * <ul>
	 * <li>Load an FXML file from the {@link InputStream} generated by the given
	 * {@link Supplier}</li>
	 * <li>Set up a scene for the loaded FXML stuff</li>
	 * <li>If the controller extends {@link AbstractWindowController} the
	 * controllers extra functionality will be initialized</li>
	 * <li>add icons to the stage based on the provided {@link Function}</li>
	 * </ul>
	 *
	 * @param <T>                     the type of the controller class
	 * @param stage                   the stage to use for the scene
